...
void device_harness(void) {
  ...
  unit_assert_ok(xDeviceWrite(0xFFu, &bytes1, data1));
  unit_assert_ok(xMemFree(data1));
  unit_end();
  unit_begin("Device availability check returns true for active device");
  unit_assert_ok(xDeviceIsAvailable(0xFFu, &res));
  unit_assert_true(res);
  unit_end();
  unit_begin("Device read retrieves previously written data");
  bytes2 = nil;
  data2 = null;
  unit_assert_ok(xDeviceRead(0xFFu, &bytes2, &data2));
  unit_assert_equal(bytes2, 0x26u);
  unit_assert_equal(strncmp((char *) data2, "THIS IS A TEST OF THE LOOPBACK DEVICE\0", bytes2), nil);
  unit_assert_ok(xMemFree(data2));
  unit_end();
  unit_begin("Simple device write sends single byte");
  data3 = 0xFAu;
  unit_assert_ok(xDeviceSimpleWrite(0xFFu, data3));
  unit_end();
  unit_begin("Simple device read retrieves single byte");
  data4 = nil;
  unit_assert_ok(xDeviceSimpleRead(0xFFu, &data4));
  unit_assert_equal(data4, 0xFAu);
  unit_end();
