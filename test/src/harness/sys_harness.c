...
static void test_system_information(void) {
  ...
  unit_assert_true(__strnlen__(info->productName, OS_PRODUCT_NAME_SIZE) >= PRODUCT_NAME_MIN_LENGTH);
  ...
  unit_begin("Multiple system info retrievals work correctly");
  {
    ...
    unit_assert_equal(__strncmp__(info1->productName, info2->productName, OS_PRODUCT_NAME_SIZE), 0x0u);
    unit_assert_equal(__strncmp__(info2->productName, info3->productName, OS_PRODUCT_NAME_SIZE), 0x0u);
...
static void test_system_info_content_validation(void) {
  ...
  unit_assert_true(__strnlen__(info->productName, OS_PRODUCT_NAME_SIZE) > 0x0u);
  unit_assert_ok(xMemFree(info));
  unit_end();
...
static void test_concurrent_system_operations(void) {
  ...
  unit_begin("Concurrent system info retrievals work correctly");
  {
    ...
    for(i = 0x0u; i < 24; i++) {
      unit_assert_equal(__strncmp__(infoArray[i]->productName, infoArray[i + 1]->productName, OS_PRODUCT_NAME_SIZE), 0x0u);
    }
