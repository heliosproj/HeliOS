...
void task_harness(void) {
  ...
  task01 = null;
  unit_try(ISSUCCESSFUL(xTaskCreate(&task01, (Byte_t *) "TASK01", task_harness_task, null)));
  unit_try(null != task01);
  unit_end();
  unit_begin("xTaskGetHandleByName()");
  task02 = null;
  unit_try(ISSUCCESSFUL(xTaskGetHandleByName(&task02, (Byte_t *) "TASK01")));
  unit_try(task02 == task01);
  unit_end();
  unit_begin("xTaskGetHandleById()");
  task02 = null;
  task03 = 0x0;
  unit_try(ISSUCCESSFUL(xTaskGetId(task01, &task03)));
  unit_try(ISSUCCESSFUL(xTaskGetHandleById(&task02, task03)));
  unit_try(task02 == task01);
  unit_end();
  unit_begin("xTaskGetAllRunTimeStats()");
  task04 = null;
  task05 = 0;
  unit_try(ISSUCCESSFUL(xTaskGetAllRunTimeStats(&task04, &task05)));
  ...
  task04 = null;
  unit_try(ISSUCCESSFUL(xTaskGetTaskRunTimeStats(task01, &task04)));
  unit_try(null != task04);
  ...
  task07 = null;
  unit_try(ISSUCCESSFUL(xTaskGetTaskInfo(task01, &task07)));
  ...
  task07 = null;
  unit_try(ISSUCCESSFUL(xTaskGetAllTaskInfo(&task07, &task06)));
  unit_try(null != task07);
  unit_try(0x1 == task06);
  unit_try(0x1 == task07->id);
  unit_try(0x0 == strncmp("TASK01", (char *) task07->name, 0x6));
  ...
  unit_try(ISSUCCESSFUL(xTaskGetTaskState(task01, &task13)));
  unit_try(TaskStateSuspended == task13);
  unit_end();
  unit_begin("xTaskGetName()");
  unit_try(ISSUCCESSFUL(xTaskGetName(task01, &task08)));
  unit_try(null != task08);
  unit_try(0x0 == strncmp("TASK01", (char *) task08, 0x6));
  unit_end();
  unit_begin("xTaskGetId()");
  unit_try(ISSUCCESSFUL(xTaskGetId(task01, &task14)));
  unit_try(0x1 == task14);
  unit_end();
  unit_begin("xTaskNotifyGive()");
  unit_try(ISSUCCESSFUL(xTaskNotifyGive(task01, 0x7, (Byte_t *) "MESSAGE")));
  unit_end();
  unit_begin("xTaskNotificationIsWaiting()");
  unit_try(ISSUCCESSFUL(xTaskNotificationIsWaiting(task01, &task15)));
  unit_try(true == task15);
  unit_end();
  unit_begin("xTaskNotifyStateClear()");
  unit_try(ISSUCCESSFUL(xTaskNotifyStateClear(task01)));
  unit_try(ISSUCCESSFUL(xTaskNotificationIsWaiting(task01, &task16)));
  unit_try(false == task16);
  unit_end();
  unit_begin("xTaskNotifyTake()");
  task09 = null;
  unit_try(ISSUCCESSFUL(xTaskNotifyGive(task01, 0x7, (Byte_t *) "MESSAGE")));
  unit_try(ISSUCCESSFUL(xTaskNotifyTake(task01, &task09)));
  ...
  unit_try(ISSUCCESSFUL(xTaskResume(task01)));
  unit_try(ISSUCCESSFUL(xTaskGetTaskState(task01, &task17)));
  unit_try(TaskStateRunning == task17);
  unit_end();
  unit_begin("xTaskSuspend()");
  unit_try(ISSUCCESSFUL(xTaskSuspend(task01)));
  unit_try(ISSUCCESSFUL(xTaskGetTaskState(task01, &task18)));
  unit_try(TaskStateSuspended == task18);
  unit_end();
  unit_begin("xTaskWait()");
  unit_try(ISSUCCESSFUL(xTaskWait(task01)));
  unit_try(ISSUCCESSFUL(xTaskGetTaskState(task01, &task19)));
  unit_try(TaskStateWaiting == task19);
  unit_end();
  unit_begin("xTaskChangePeriod()");
  unit_try(ISSUCCESSFUL(xTaskChangePeriod(task01, 0xD05)));
  unit_try(ISSUCCESSFUL(xTaskGetPeriod(task01, &task20)));
  unit_try(0xD05 == task20);
  unit_end();
  unit_begin("xTaskGetPeriod()");
  unit_try(ISSUCCESSFUL(xTaskChangePeriod(task01, 0x1E61)));
  unit_try(ISSUCCESSFUL(xTaskGetPeriod(task01, &task21)));
  unit_try(0x1E61 == task21);
  unit_end();
  unit_begin("xTaskResetTimer()");
  unit_try(ISSUCCESSFUL(xTaskResetTimer(task01)));
  unit_end();
  unit_begin("xTaskGetSchedulerState()");
  unit_try(ISSUCCESSFUL(xTaskSuspendAll()));
  unit_try(ISSUCCESSFUL(xTaskGetSchedulerState(&task22)));
  ...
  unit_try(ISSUCCESSFUL(xTaskChangePeriod(task10, 0xBB8)));
  unit_try(ISSUCCESSFUL(xTaskWait(task10)));
  unit_try(ISSUCCESSFUL(xTaskResetTimer(task10)));
  unit_try(ISSUCCESSFUL(xTaskStartScheduler()));
  unit_try(ISSUCCESSFUL(xTaskDelete(task10)));
  unit_end();
  unit_begin("Unit test for direct to task notification event");
  unit_try(ISSUCCESSFUL(xTaskResumeAll()));
  task11 = null;
  unit_try(ISSUCCESSFUL(xTaskCreate(&task11, (Byte_t *) "TASK11", task_harness_task, null)));
  unit_try(null != task11);
  unit_try(ISSUCCESSFUL(xTaskWait(task11)));
  unit_try(ISSUCCESSFUL(xTaskNotifyGive(task11, 0x7, (Byte_t *) "MESSAGE")));
  unit_try(ISSUCCESSFUL(xTaskNotificationIsWaiting(task11, &task24)));
  unit_try(true == task24);
  unit_try(ISSUCCESSFUL(xTaskResumeAll()));
  unit_try(ISSUCCESSFUL(xTaskStartScheduler()));
  unit_try(ISSUCCESSFUL(xTaskNotificationIsWaiting(task11, &task25)));
  unit_try(false == task25);
  unit_try(ISSUCCESSFUL(xTaskDelete(task11)));
  unit_end();
  unit_begin("Unit test for task watchdog timer");
  unit_try(ISSUCCESSFUL(xTaskResumeAll()));
  task12 = null;
  unit_try(ISSUCCESSFUL(xTaskCreate(&task12, (Byte_t *) "TASK12", task_harness_task2, null)));
