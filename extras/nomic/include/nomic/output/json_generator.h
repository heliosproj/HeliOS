...
public: JSONOutputGenerator();
     ...
     */
    void generateReport(const std::vector < Violation > &violations, const OutputOptions &options, std::ostream &output);
     ...
     */
    void generateReportToFile(const std::vector < Violation > &violations, const OutputOptions &options, const std::string &filename);
...
private:
     ...
     * @return           JSON object
     */
    nlohmann::json violationToJSON(const Violation &violation, const OutputOptions &options) const;
     ...
     */
    std::string extractSourceSnippet(const SourceLocation &loc, unsigned context_lines = 3) const;
  };
 ...
 */
  class OutputManager {
public: explicit OutputManager(const std::string &format = "json");
     ...
     * @param output     Output stream
     */
    void write(const std::vector < Violation > &violations, std::ostream &output);
     ...
     * @param filename   Output filename
     */
    void writeToFile(const std::vector < Violation > &violations, const std::string &filename);
