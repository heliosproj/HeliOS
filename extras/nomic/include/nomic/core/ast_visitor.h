...
public: explicit SemanticASTVisitor(clang::ASTContext &ctx, SemanticDatabase &db);
    ...
    void HandleMacroDefinition(const clang::Token &macro_name_tok, const clang::MacroDirective *md);
    void HandleInclusionDirective(clang::SourceLocation hash_loc, const clang::Token &include_tok, llvm::StringRef file_name, bool is_angled, clang::
      CharSourceRange filename_range, const clang::FileEntry *file, llvm::StringRef search_path, llvm::StringRef relative_path, const clang::Module *imported);
...
private: clang::ASTContext & ast_context_;
...
public: explicit SemanticASTConsumer(clang::ASTContext &context, SemanticDatabase &db) : visitor_(context, db) {
    } void HandleTranslationUnit(clang::ASTContext &context) override {
      visitor_.TraverseDecl(context.getTranslationUnitDecl());
    }


private: SemanticASTVisitor visitor_;
  };
...
public: explicit SemanticAnalysisAction(SemanticDatabase &db) : semantic_db_(db) {
