...
/**
 * @brief Rule evaluator for DSL expressions Note: Defined here because
 * RuleEngine uses unique_ptr which needs complete type
 */
  class RuleEvaluator {
 ...
 */
  class Rule {
public: Rule(const std::string &id, const std::string &description, Severity severity) : id_(id), description_(description), severity_(severity) {
    }
    ...
    void setSelectExpression(const std::string &expr) {
      select_expression_ = expr;
    }


    void setWhereClause(const std::string &clause) {
      where_clause_ = clause;
    }


    void setAssertExpression(const std::string &expr) {
      assert_expression_ = expr;
    }
    ...
    void setFixit(const std::string &fixit) {
      fixit_ = fixit;
    }
    ...
    const std::string & getSelectExpression() const {
      return(select_expression_);
    }


    const std::optional < std::string > &getWhereClause() const {
      return(where_clause_);
    } const std::string & getAssertExpression() const {
      return(assert_expression_);
    }
    ...
    const std::optional < std::string > &getFixit() const {
      return(fixit_);
 ...
 */
  class Violation {
public: Violation(const Rule &rule, const SourceRange &location, const std::string &message) : rule_id_(rule.getId()), severity_(rule.getSeverity()), location_(
        location), message_(message) {
    }
    ...
    void setSnippet(const std::string &snippet) {
      snippet_ = snippet;
    }
    ...
    const std::optional < std::string > &getContextScope() const {
      return(context_scope_);
    } const std::optional < std::string > &getContextName() const {
      return(context_name_);
    } const std::optional < std::string > &getSnippet() const {
      return(snippet_);
    } const std::optional < std::string > &getSuggestedFixDescription() const {
      return(suggested_fix_description_);
    } const std::optional < std::string > &getSuggestedFixDiff() const {
      return(suggested_fix_diff_);
    } private: std::string rule_id_;
    Severity severity_;
...
public: void invalidate() {
    ...
    template < typename T > void cacheEntity(const std::string &key, T *entity) {
      // Implementation in source file
    }


    template < typename T > T * getCachedEntity(const std::string &key) const {
      // Implementation in source file
      return(nullptr);
    }
    ...
    std::optional < bool > getCachedEvaluation(const std::string &key) const {
      ...
      return(std::nullopt);
    } private: std::map < std::string, void * > entity_cache_;
...
public: RuleEngine();
    ...
    void clearRules();
    // Rule execution
    std::vector < Violation > executeRules(const RuleSet &rules, const SemanticDatabase &db, const ExecutionOptions &options = {});
    std::vector < Violation > executeRule(const Rule &rule, const SemanticDatabase &db);
    // Configuration
    ...
    const ExecutionStats & getLastExecutionStats() const {
      return(last_stats_);
    }
    void printStatistics(std::ostream &out = std::cout) const;
private:
    // Parallel execution support
    std::vector < Violation > executeRulesParallel(const RuleSet &rules, const SemanticDatabase &db, unsigned num_threads);
    // Sequential execution
    std::vector < Violation > executeRulesSequential(const RuleSet &rules, const SemanticDatabase &db);
    // Rule filtering
    std::vector < Rule > filterRules(const RuleSet &rule_set) const;
    // Entity selection
    template < typename T > std::vector < T * > selectEntities(const std::string &scope, const SemanticDatabase &db) const;
...
private: std::vector < Rule > rules_;
