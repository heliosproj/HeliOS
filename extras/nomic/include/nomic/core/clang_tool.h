...
public: explicit SemanticAnalysisFrontendActionFactory(SemanticDatabase &db) : semantic_db_(db) {
    } std::unique_ptr < clang::FrontendAction > create() override {
      return(std::make_unique < SemanticAnalysisAction > (semantic_db_));
    } private: SemanticDatabase & semantic_db_;
  };
...
public: ClangAnalyzer();
     ...
     */
    bool analyzeFile(const std::string &source_file, const std::vector < std::string > &compile_args = {});
    /**
     ...
     * @return              true on success, false on failure
     */
    bool analyzeFiles(const std::vector < std::string > &source_files, const std::vector < std::string > &compile_args = {});
    /**
     ...
     * @return                     true on success, false on failure
     */
    bool analyzeWithCompilationDatabase(const std::string &compilation_db_path, const std::vector < std::string > &source_files = {});
     ...
     */
    SemanticDatabase & getSemanticDatabase() {
      return(semantic_db_);
    }


    const SemanticDatabase & getSemanticDatabase() const {
      return(semantic_db_);
    }
    /**
...
private:
     ...
     */
    int runTool(const std::vector < std::string > &sources, const std::vector < std::string > &args);
...
public: struct CompileCommand {
     ...
     */
    void addCommand(const std::string &file, const std::string &directory, const std::vector < std::string > &args);
    /**
     ...
     */
    static std::string createSimple(const std::vector < std::string > &files, const std::vector < std::string > &compile_args, const std::string &working_dir =
      ".");
private: std::vector < CompileCommand > commands_;
  };
