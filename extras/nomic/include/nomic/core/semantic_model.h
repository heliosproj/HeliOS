...
public:
    void addDefine(const std::string& define) { active_defines_.push_back(define); }
    void setGuardCondition(const std::string& guard) { guard_condition_ = guard; }
    void setInMacroExpansion(bool in_macro) { in_macro_expansion_ = in_macro; }
    void setExpandedFrom(const std::string& macro) { expanded_from_ = macro; }

    const std::vector<std::string>& getActiveDefines() const { return active_defines_; }
    const std::optional<std::string>& getGuardCondition() const { return guard_condition_; }
    bool isInMacroExpansion() const { return in_macro_expansion_; }
    const std::optional<std::string>& getExpandedFrom() const { return expanded_from_; }

private:
    std::vector<std::string> active_defines_;
...
public:
    TypeInfo() = default;
    explicit TypeInfo(const std::string& canonical_type)
        : canonical_type_(canonical_type) {}

    // Setters
    void setCanonicalType(const std::string& type) { canonical_type_ = type; }
    void setDesugaredType(const std::string& type) { desugared_type_ = type; }
    void setIsPointer(bool is_ptr) { is_pointer_ = is_ptr; }
    void setIsArray(bool is_array) { is_array_ = is_array; }
    void setIsConst(bool is_const) { is_const_ = is_const; }
    void setIsVolatile(bool is_volatile) { is_volatile_ = is_volatile; }
    void setIsStatic(bool is_static) { is_static_ = is_static; }
    void setPointerDepth(unsigned depth) { pointer_depth_ = depth; }
    void setArraySize(size_t size) { array_size_ = size; }

    // Getters
    const std::string& getCanonicalType() const { return canonical_type_; }
    const std::string& getDesugaredType() const { return desugared_type_; }
    bool isPointer() const { return is_pointer_; }
    bool isArray() const { return is_array_; }
    bool isConst() const { return is_const_; }
    bool isVolatile() const { return is_volatile_; }
    bool isStatic() const { return is_static_; }
    unsigned getPointerDepth() const { return pointer_depth_; }
    const std::optional<size_t>& getArraySize() const { return array_size_; }
...
public:
    ...
    Statement(Type type = OTHER, const SourceRange& loc = SourceRange(), const std::string& text = "")
        : type_(type), location_(loc), text_(text) {}
...
class CallSite {
public:
    CallSite(const std::string& name = "", const SourceLocation& loc = SourceLocation())
        : name_(name), location_(loc) {}

    // Getters
    const std::string& getName() const { return name_; }
    const SourceLocation& getLocation() const { return location_; }
    const std::string& getQualifiedName() const { return qualified_name_; }
    const std::string& getExpressionText() const { return expression_text_; }
    unsigned getArgumentCount() const { return argument_count_; }
    const std::vector<std::string>& getArgumentExpressions() const { return argument_expressions_; }
    bool isVirtual() const { return is_virtual_; }
    bool isIndirect() const { return is_indirect_; }

    // Setters
    void setQualifiedName(const std::string& name) { qualified_name_ = name; }
    void setExpressionText(const std::string& text) { expression_text_ = text; }
    void setArgumentCount(unsigned count) { argument_count_ = count; }
    void addArgumentExpression(const std::string& arg) { argument_expressions_.push_back(arg); }
    void setIsVirtual(bool is_virtual) { is_virtual_ = is_virtual; }
    void setIsIndirect(bool is_indirect) { is_indirect_ = is_indirect; }

private:
    std::string name_;                           // Simple function name
...
class ArrayAccess {
public:
    ArrayAccess(const std::string& array_name = "", const SourceLocation& loc = SourceLocation())
        : array_name_(array_name), location_(loc) {}

    // Getters
    const std::string& getArrayName() const { return array_name_; }
    const SourceLocation& getLocation() const { return location_; }
    const std::string& getIndexExpression() const { return index_expression_; }
    const std::string& getFullExpression() const { return full_expression_; }
    bool isWrite() const { return is_write_; }

    // Setters
    void setIndexExpression(const std::string& index) { index_expression_ = index; }
    void setFullExpression(const std::string& expr) { full_expression_ = expr; }
    void setIsWrite(bool is_write) { is_write_ = is_write; }

private:
    std::string array_name_;           // Name of the array being accessed
...
public:
    ReturnStatement(const SourceLocation& loc = SourceLocation())
        : location_(loc) {}

    // Getters
    const SourceLocation& getLocation() const { return location_; }
    const std::string& getReturnExpression() const { return return_expression_; }
    bool hasValue() const { return has_value_; }

    // Setters
    void setReturnExpression(const std::string& expr) { return_expression_ = expr; }
    void setHasValue(bool has_value) { has_value_ = has_value; }
...
class Annotation {
public:
    Annotation(const std::string& name = "", const std::string& value = "")
...
class Variable {
public:
    Variable(const std::string& name, const TypeInfo& type, const SourceRange& location)
    ...
    void setTainted(bool is_tainted) { is_tainted_ = is_tainted; }
    void setTaintSource(const std::string& source) { taint_source_ = source; }
    void setTaintConfidence(float confidence) { taint_confidence_ = confidence; }
    ...
    bool isTainted() const { return is_tainted_; }
    const std::optional<std::string>& getTaintSource() const { return taint_source_; }
    float getTaintConfidence() const { return taint_confidence_; }
...
private:
    ...
    std::optional<std::string> taint_source_;  // Source function/origin of taint
...
public:
    ...
    // Setters
    void setLineCount(size_t count) { line_count_ = count; }
    void setFunctionCount(size_t count) { function_count_ = count; }
    void setGlobalVarCount(size_t count) { global_var_count_ = count; }
    void setIncludeCount(size_t count) { include_count_ = count; }
    void setHasHeaderGuard(bool has_guard) { has_header_guard_ = has_guard; }
    void addSystemInclude(const std::string& include) { system_includes_.push_back(include); }

    void incrementFunctionCount() { function_count_++; }
    void incrementGlobalVarCount() { global_var_count_++; }
    void incrementIncludeCount() { include_count_++; }
    ...
    size_t getFunctionCount() const { return function_count_; }
    size_t getGlobalVarCount() const { return global_var_count_; }
    size_t getIncludeCount() const { return include_count_; }
    bool hasHeaderGuard() const { return has_header_guard_; }
    bool isHeader() const { return is_header_; }
    const std::vector<std::string>& getSystemIncludes() const { return system_includes_; }
...
class Function {
public:
    Function(const std::string& name, const std::string& signature, const SourceRange& location)
        : name_(name), signature_(signature), location_(location) {}

    // Setters
    void setReturnType(const TypeInfo& type) { return_type_ = type; }
    void addParameter(const Variable& param) { parameters_.push_back(param); }
    void addLocalVariable(const Variable& var) { local_vars_.push_back(var); }
    void addCallSite(const CallSite& call);
    void addArrayAccess(const ArrayAccess& access) { array_accesses_.push_back(access); }
    void addReturnStatement(const ReturnStatement& ret) { return_statements_.push_back(ret); }
    void setCFG(std::unique_ptr<ControlFlowGraph> cfg);
    void addStatement(const Statement& stmt);
    void setCyclomaticComplexity(unsigned complexity) { cyclomatic_complexity_ = complexity; }
    void setPreprocessorContext(const PreprocessorContext& ctx) { preprocessor_ = ctx; }
    void addAnnotation(const Annotation& annotation);
    void setStatic(bool is_static) { is_static_ = is_static; }
    void setInline(bool is_inline) { is_inline_ = is_inline; }
    void setLinkage(const std::string& linkage) { linkage_ = linkage; }

    // Getters
    const std::string& getName() const { return name_; }
    const std::string& getSignature() const { return signature_; }
    const TypeInfo& getReturnType() const { return return_type_; }
    const std::vector<Variable>& getParameters() const { return parameters_; }
    const std::vector<Variable>& getLocalVariables() const { return local_vars_; }
    const std::vector<CallSite>& getCallSites() const { return calls_; }
    const std::vector<ArrayAccess>& getArrayAccesses() const { return array_accesses_; }
    const std::vector<ReturnStatement>& getReturnStatements() const { return return_statements_; }
    ControlFlowGraph* getCFG() const { return cfg_.get(); }
    const std::vector<Statement>& getStatements() const { return statements_; }
    unsigned getCyclomaticComplexity() const { return cyclomatic_complexity_; }
    const SourceRange& getLocation() const { return location_; }
    const PreprocessorContext& getPreprocessorContext() const { return preprocessor_; }
    const std::vector<Annotation>& getAnnotations() const { return annotations_; }
    bool isStatic() const { return is_static_; }
    bool isInline() const { return is_inline_; }
    const std::string& getLinkage() const { return linkage_; }
...
public:
    ...
    // Setters
    void addStatement(const Statement& stmt);
    void addPredecessor(unsigned pred_id) { predecessors_.insert(pred_id); }
    void addSuccessor(unsigned succ_id) { successors_.insert(succ_id); }
    void setEntry(bool is_entry) { is_entry_ = is_entry; }
    void setExit(bool is_exit) { is_exit_ = is_exit; }
    void setCondition(const std::string& condition) { condition_ = condition; }

    // Getters
    unsigned getId() const { return id_; }
    const std::vector<Statement>& getStatements() const { return statements_; }
    const std::set<unsigned>& getPredecessors() const { return predecessors_; }
    const std::set<unsigned>& getSuccessors() const { return successors_; }
    bool isEntry() const { return is_entry_; }
    bool isExit() const { return is_exit_; }
    const std::optional<std::string>& getCondition() const { return condition_; }
...
public:
    ...
    const std::map<unsigned, std::set<unsigned>>& getDominanceTree() const { return dominance_tree_; }
    const std::map<unsigned, unsigned>& getImmediateDominators() const { return immediate_dominators_; }
...
private:
    ...
    std::map<unsigned, std::set<std::string>> reaching_defs_in_;   // Reaching definitions at entry
    std::map<unsigned, std::set<std::string>> reaching_defs_out_;  // Reaching definitions at exit
    std::map<unsigned, std::set<std::string>> live_vars_in_;       // Live variables at entry
    std::map<unsigned, std::set<std::string>> live_vars_out_;      // Live variables at exit
    ...
    std::map<unsigned, std::set<std::string>> gen_defs_;  // Variables defined in block
    std::map<unsigned, std::set<std::string>> kill_defs_; // Definitions killed in block
    std::map<unsigned, std::set<std::string>> gen_uses_;  // Variables used in block
    std::map<unsigned, std::set<std::string>> def_sites_; // Blocks where variable is defined
    std::map<unsigned, std::set<std::string>> use_sites_; // Blocks where variable is used

    // Taint analysis results
    std::map<unsigned, std::set<std::string>> tainted_vars_in_;   // Tainted variables at entry
    std::map<unsigned, std::set<std::string>> tainted_vars_out_;  // Tainted variables at exit
    std::map<std::string, std::vector<std::string>> taint_paths_; // Taint propagation paths
};
...
class PatternMatch {
public:
    PatternMatch(const SourceRange& location, const std::string& matched_text)
        : location_(location), matched_text_(matched_text) {}

    void addCapture(const std::string& name, const std::string& value) {
        captures_[name] = value;
    }
    void setConfidenceScore(double score) { confidence_score_ = score; }

    const SourceRange& getLocation() const { return location_; }
    const std::map<std::string, std::string>& getCaptures() const { return captures_; }
    const std::string& getMatchedText() const { return matched_text_; }
...
public:
    ...
    const std::vector<std::unique_ptr<Variable>>& getGlobalVariables() const { return global_variables_; }
    const std::map<std::string, TypeInfo>& getTypeDefinitions() const { return type_definitions_; }
    const std::map<std::string, std::unique_ptr<FileInfo>>& getFiles() const { return files_; }
