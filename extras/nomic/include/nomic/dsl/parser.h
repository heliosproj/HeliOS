 ...
 *
 * Grammar (simplified):
 * expression := quantifier | logical_or quantifier := (ALL|ANY|EXISTS|NONE) '('
 * IDENT 'in' expression ['where' expression] 'satisfies' expression ')'
 * logical_or := logical_and ('or' logical_and)*
 * logical_and := comparison ('and' comparison)*
 * comparison := additive (('=='|'!='|'<'|'<='|'>'|'>=') additive)?
 * additive := multiplicative (('+'|'-') multiplicative)*
 * multiplicative := unary (('*'|'/') unary)*
 * unary := ('not' | '-') unary | postfix postfix := primary ('.' IDENT | '('
 * arglist ')')*
 ...
 */
  class Parser {
public: explicit Parser(const std::vector < Token > &tokens);
    std::unique_ptr < ASTNode > parse();
private: const std::vector < Token > &tokens_;
    size_t current_;
