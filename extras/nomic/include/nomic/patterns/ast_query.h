...
public: ASTQueryPredicate(const std::string &attr, ASTQueryOperator op, const std::string &value) : attribute_(attr), operator_(op), value_(value) {
    } const std::string & getAttribute() const {
      return(attribute_);
    }


    ASTQueryOperator getOperator() const {
      return(operator_);
    }
...
private: std::string attribute_; // Attribute name (e.g., "name", "type")
...
public: ASTQueryCapture(const std::string &name) : name_(name) {
    } const std::string & getName() const {
      return(name_);
    }


    void addNode(const clang::Stmt *node) {
      captured_nodes_.push_back(node);
    }


    void addDecl(const clang::Decl *decl) {
      captured_decls_.push_back(decl);
    }


    const std::vector < const clang::Stmt * > &getNodes() const {
      return(captured_nodes_);
    } const std::vector < const clang::Decl * > &getDecls() const {
      return(captured_decls_);
 ...
 */
  class ASTQueryNode {
public: ASTQueryNode(ASTQueryNodeType type) : type_(type) {
    }


    // Setters
    void setNodeType(ASTQueryNodeType type) {
      type_ = type;
    }


    void addPredicate(const ASTQueryPredicate &pred) {
      predicates_.push_back(pred);
    }


    void addChild(std::unique_ptr < ASTQueryNode > child) {
      children_.push_back(std::move(child));
    }


    void setCombinator(ASTQueryNodeType comb) {
      combinator_ = comb;
    }
    ...
    void setQuantifier(const std::string &quant) {
      quantifier_ = quant;
    }                                                                     // *,
                                                                          // +,
                                                                          // ?
    ...
    // Getters
    ASTQueryNodeType getNodeType() const {
      return(type_);
    }


    const std::vector < ASTQueryPredicate > &getPredicates() const {
      return(predicates_);
    } const std::vector < std::unique_ptr < ASTQueryNode >> &getChildren() const {
      return(children_);
    } std::optional < ASTQueryNodeType > getCombinator() const {
      return(combinator_);
    } const std::optional < std::string > &getCaptureName() const {
    ...
    const std::optional < std::string > &getQuantifier() const {
      return(quantifier_);
    } private: ASTQueryNodeType type_;
 ...
 */
  class ASTQueryMatch {
public: void addCapture(const std::string &name, const clang::Stmt *node);
    ...
    } private: std::map < std::string, std::vector < const clang::Stmt * >> node_captures_;
 ...
 *    call_expr[@name="malloc"] @alloc ~ (!call_expr[@name="free"]) ...
 * return_stmt
 */
  class ASTQueryParser {
public: ASTQueryParser() = default;
...
public: ASTQueryMatcher(const ASTQueryNode &query) : query_(query) {
    }
...
private: const ASTQueryNode &query_;
    ...
    bool matchPredicates(const std::vector < ASTQueryPredicate > &predicates, const std::map < std::string, std::string > &attributes);
...
private: ASTQueryParser parser_;
  };
 ...
 */
  class ASTTemplate {
public: ASTTemplate() = default;
    ASTTemplate(const std::string &name, const std::string &description) : name_(name), description_(description) {
    ...
    void setFixTemplate(const std::string &fix_template) {
...
public: static ASTTemplateLibrary & instance();
...
private: ASTTemplateLibrary() = default;
