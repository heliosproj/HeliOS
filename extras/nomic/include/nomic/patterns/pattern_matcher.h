 ...
 */
  struct MultiLinePattern {
    ...
    unsigned max_gap_lines = 0; // Maximum lines between pattern matches (0 = no
                                // limit)
    ...
    std::map < size_t, std::vector < size_t >> ordering_constraints; // line_index
                                                                     // ->
                                                                     // must_precede_indices
    ...
    std::map < size_t, LineQuantifier > line_quantifiers; // line_index ->
                                                          // quantifier
 ...
 */
  struct MatchResult {
    ...
    std::map < std::string, std::vector < std::string >> multi_captures; // For
                                                                         // quantified
                                                                         // captures
...
/**
 * @brief Multi-line pattern matcher with semantic awareness and advanced
 * features
 */
  class MultiLinePatternMatcher {
public: MultiLinePatternMatcher();
    ~MultiLinePatternMatcher();
    // ==================== BASIC PATTERN MATCHING ====================
    // Pattern matching
    std::vector < MatchResult > findMatches(const MultiLinePattern &pattern, const std::vector < Statement > &statements);
    std::vector < MatchResult > findMatchesInFunction(const MultiLinePattern &pattern, const Function &func);
    // Semantic-aware matching
    std::vector < MatchResult > findSemanticMatches(const MultiLinePattern &pattern, const Function &func, const SemanticConstraints &constraints);
    // ==================== ADVANCED MATCHING MODES ====================
    // Fuzzy matching with configurable threshold
    std::vector < MatchResult > findFuzzyMatches(const MultiLinePattern &pattern, const std::vector < Statement > &statements, float min_similarity = 0.7f);
    // Structural matching (ignores variable names, literals)
    std::vector < MatchResult > findStructuralMatches(const MultiLinePattern &pattern, const std::vector < Statement > &statements);
    // Wildcard matching (supports * and ?)
    std::vector < MatchResult > findWildcardMatches(const MultiLinePattern &pattern, const std::vector < Statement > &statements);
    // Unordered matching (finds pattern lines in any order)
    std::vector < MatchResult > findUnorderedMatches(const MultiLinePattern &pattern, const std::vector < Statement > &statements);
    // Gap-tolerant matching (allows gaps between pattern lines)
    std::vector < MatchResult > findGapTolerantMatches(const MultiLinePattern &pattern, const std::vector < Statement > &statements, unsigned max_gap = 5);
    // ==================== QUANTIFIED MATCHING ====================
    // Match with line quantifiers (e.g., "if (...)*" matches multiple ifs)
    std::vector < MatchResult > findQuantifiedMatches(const MultiLinePattern &pattern, const std::vector < Statement > &statements);
    // ==================== CONTEXT-AWARE MATCHING ====================
    // Match with before/after context requirements
    std::vector < MatchResult > findContextualMatches(const MultiLinePattern &pattern, const std::vector < Statement > &statements, const MultiLinePattern &
      before_context, const MultiLinePattern &after_context);
    // Match within specific scope (loop, if block, etc.)
    std::vector < MatchResult > findScopedMatches(const MultiLinePattern &pattern, const Function &func, const std::string &scope_type); // "loop",
                                                                                                                                         // "if",
                                                                                                                                         // "switch",
                                                                                                                                         // etc.
    // ==================== PATTERN TRANSFORMATION ====================
    // Extract all unique matches of a pattern
    std::vector < std::string > extractPatternInstances(const MultiLinePattern &pattern, const std::vector < Statement > &statements);
    // Replace matched patterns with new code
    std::string replaceMatches(const MultiLinePattern &pattern, const std::vector < Statement > &statements, const std::string &replacement_template);
    // ==================== PATTERN COMPOSITION ====================
...
private:
    ...
    bool matchLine(const std::string &line, const std::regex &pattern_regex, std::map < std::string, std::string > &captures) const;
    bool matchLineWildcard(const std::string &line, const std::string &wildcard_pattern, std::map < std::string, std::string > &captures) const;
    bool matchLineFuzzy(const std::string &line, const std::string &pattern, float min_similarity) const;
    bool matchLineStructural(const std::string &line, const std::string &pattern) const;
    // Semantic constraint checking
    bool checkSemanticConstraints(const std::vector < Statement > &matched_statements, const SemanticConstraints &constraints, const Function &func) const;
    ...
    size_t countGapStatements(const std::vector < Statement > &statements, size_t start_idx, size_t end_idx) const;
    // Quantifier matching
    ...
    std::vector < QuantifierMatch > matchWithQuantifier(const std::vector < Statement > &statements, size_t start_idx, const std::regex &pattern, const
      MultiLinePattern::LineQuantifier &quantifier) const;
    // Group matching
    bool matchLineGroup(const std::vector < Statement > &statements, size_t &current_idx, const MultiLinePattern::LineGroup &group, const MultiLinePattern &
      pattern, std::map < std::string, std::string > &captures) const;
    // Ordering validation
    bool validateOrdering(const std::vector < size_t > &matched_indices, const std::map < size_t, std::vector < size_t >> &ordering_constraints) const;
    // Context extraction
    std::string extractContext(const std::vector < Statement > &statements, size_t match_start, size_t match_end, size_t context_lines = 3) const;
...
private: CompiledPattern compiled_pattern_;
    mutable std::map < std::string, float > similarity_cache_; // Cache for
                                                               // expensive
                                                               // similarity
                                                               // calculations
  };
// Forward declarations for AST pattern matching are already at the top of the
// file
...
public: explicit ASTPatternMatcher(clang::ASTContext &context);
    ~ASTPatternMatcher();
    // Pattern matching
    ...
    ASTPattern combinePatterns(const ASTPattern &p1, const ASTPattern &p2, CombineMode mode);
    // Pattern validation
    bool validatePattern(const ASTPattern &pattern) const;
private: class Impl;
    std::unique_ptr < Impl > impl_;
  };
// Forward declarations for IR pattern matching are already at the top of the
// file
...
public: IRPatternMatcher();
    ~IRPatternMatcher();
    // Pattern matching
    std::vector < llvm::Instruction * > matchInstructions(const IRPattern &pattern, llvm::Function &func);
    // SSA-aware matching
    std::vector < std::pair < llvm::Value *, llvm::Value * >> matchDefUseChains(const IRPattern &def_pattern, const IRPattern &use_pattern, llvm::Function &func
      );
      // Pattern validation
...
public: UnifiedPatternMatcher(clang::ASTContext * ast_context = nullptr);
    ...
    UnifiedMatchResult match(const UnifiedPattern &pattern, const Function &func);
    // Pattern learning
    UnifiedPattern learnPatternFromExamples(const std::vector < std::string > &examples);
private: std::unique_ptr < MultiLinePatternMatcher > text_matcher_;
