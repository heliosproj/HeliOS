...
public: NamingConvention() = default;
    NamingConvention(CaseStyle style) : case_style_(style) {
    } void setCaseStyle(CaseStyle style) {
      case_style_ = style;
    }
    ...
    const std::optional < std::string > &getPrefix() const {
      return(prefix_);
    } const std::optional < std::string > &getSuffix() const {
      return(suffix_);
    } private: CaseStyle case_style_ = CaseStyle::ANY;
...
public:
    ...
    void setFunctionConvention(const NamingConvention &conv) {
      function_conv_ = conv;
    }


    void setVariableConvention(const NamingConvention &conv) {
      variable_conv_ = conv;
    }


    void setParameterConvention(const NamingConvention &conv) {
      parameter_conv_ = conv;
    }


    void setTypeConvention(const NamingConvention &conv) {
      type_conv_ = conv;
    }


    void setConstantConvention(const NamingConvention &conv) {
      constant_conv_ = conv;
    }


    void setMacroConvention(const NamingConvention &conv) {
      macro_conv_ = conv;
    }


    void setGlobalVariableConvention(const NamingConvention &conv) {
      global_var_conv_ = conv;
    }


    void setStaticVariableConvention(const NamingConvention &conv) {
      static_var_conv_ = conv;
    }


    void setEnumConstantConvention(const NamingConvention &conv) {
      enum_const_conv_ = conv;
    }


    void setStructConvention(const NamingConvention &conv) {
      struct_conv_ = conv;
    }
    // Check functions
...
private: NamingConvention function_conv_;
 ...
 */
  class DocComment {
public: void setBrief(const std::string &brief) {
      brief_ = brief;
    }


    void setDetailed(const std::string &detailed) {
      detailed_ = detailed;
    }
    void addParam(const std::string &name, const std::string &desc);


    void setReturn(const std::string &desc) {
      return_desc_ = desc;
    }
    ...
    bool hasReturnDoc() const {
      return(return_desc_.has_value());
    }
    bool hasTag(const std::string &tag) const;
    const std::optional < std::string > &getBrief() const {
      return(brief_);
    } const std::optional < std::string > &getDetailed() const {
      return(detailed_);
    } const std::map < std::string, std::string > &getParams() const {
      return(param_docs_);
    } const std::optional < std::string > &getReturn() const {
      return(return_desc_);
    } private: std::optional < std::string > brief_;
 ...
 */
  class DocChecker {
public: DocChecker(DocStyle style = DocStyle::DOXYGEN) : parser_(style) {
    } void setRequirements(const DocRequirements &reqs) {
      requirements_ = reqs;
    }


    void setStyle(DocStyle style) {
      parser_ = DocParser(style);
    }
    bool checkFunctionDoc(const std::string &comment, const std::vector < std::string > &param_names, bool has_return_value, std::vector < std::string > &
      violations) const;
    bool checkTypeDoc(const std::string &comment, std::vector < std::string > &violations) const;
private: DocParser parser_;
...
public:
    ...
    void setIncludeOrderStyle(IncludeOrderStyle style) {
      include_order_style_ = style;
    }


    void setIncludeGroups(const std::vector < std::string > &groups) {
      include_groups_ = groups;
    }
    bool checkIncludeOrder(const std::vector < Include > &includes, std::vector < std::string > &violations) const;


    // Header guard checking
    void setHeaderGuardStyle(HeaderGuardStyle style) {
      header_guard_style_ = style;
    }


    void setHeaderGuardPattern(const std::string &pattern) {
      header_guard_pattern_ = pattern;
    }
    bool checkHeaderGuard(const std::string &filename, const std::string &guard_name, std::string &violation) const;


    // Declaration order checking
    void setDeclarationOrder(const std::vector < std::string > &order) {
      declaration_order_ = order;
    }
    bool checkDeclarationOrder(const std::vector < std::string > &declarations, std::vector < std::string > &violations) const;


    // Static function placement
    void setRequireStaticFirst(bool required) {
      require_static_first_ = required;
    }
    bool checkStaticPlacement(const std::vector < std::pair < std::string, bool >> &functions, std::vector < std::string > &violations) const;
private: IncludeOrderStyle include_order_style_ = IncludeOrderStyle::SYSTEM_FIRST;
