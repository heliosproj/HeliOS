 ...
 */
  class TaintSource {
public: TaintSource(const std::string &var_name, TaintSourceType type, SourceLocation loc) : variable_name_(var_name), source_type_(type), location_(loc) {
    } const std::string & getVariableName() const {
      return(variable_name_);
    }


    TaintSourceType getSourceType() const {
      return(source_type_);
    }
 ...
 */
  class TaintSink {
public: TaintSink(const std::string &func_name, TaintSinkType type, SourceLocation loc) : function_name_(func_name), sink_type_(type), location_(loc) {
    } const std::string & getFunctionName() const {
      return(function_name_);
    }


    TaintSinkType getSinkType() const {
      return(sink_type_);
    }
    ...
    const std::vector < std::string > &getTaintedArgs() const {
      return(tainted_args_);
    } void addTaintedArg(const std::string &arg) {
      tainted_args_.push_back(arg);
    }
...
private: std::string function_name_;
 ...
 */
  class TaintFlow {
public: TaintFlow(const TaintSource &src, const TaintSink &sink) : source_(src), sink_(sink), confidence_(1.0f) {
    } const TaintSource & getSource() const {
      return(source_);
    }


    const TaintSink & getSink() const {
      return(sink_);
    }
    ...
    void addToPath(const std::string &var) {
      propagation_path_.push_back(var);
    }
    ...
    void setSanitized(bool sanitized) {
      is_sanitized_ = sanitized;
    }
