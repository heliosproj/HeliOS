 ...
 * TaskStateWaiting state. TaskState_t should be declared (i.e., used) as
 * xTaskState.
 *
 * @sa xTaskState
 * @sa xTaskResume()
 ...
 */
  typedef enum {
    ...
    /**< Returned by xTaskGetTaskState() when the task cannot be found. */
    TaskStateSuspended,


    /**< State a task is in when it is first created OR after calling
     * xTaskSuspend() - tasks in the TaskStateSuspended state will not be
     * scheduled for execution. */
    TaskStateRunning,


    /**< State a task is in after calling xTaskResume() -
     * tasks in the TaskStateRunning state will be scheduled co-operatively. */
    TaskStateWaiting /**< State a task is in after calling xTaskWait() - tasks
                      * in the TaskStateWaiting state will be scheduled as event
                      * driven. */
  } TaskState_t;
 ...
 */
  typedef enum {
     ...
     * xTaskStartScheduler() will stop scheduling tasks for execution and
 ...
 *
 * The Base_t type is a simple data type often used as an argument or return
 * type for system calls when the value is known not to exceed its 8-bit width
 * and no data structure requirements exist. There are no guarantees the Base_t
 * will always be 8-bits wide. If an 8-bit data type is needed that is
 * guaranteed to remain 8-bits wide, the Byte_t data type should be used. Base_t
 * should be declared as xBase.
 *
 * @sa xBase
 * @sa Byte_t
 *
 ...
 * between the end-user application and system calls. It is not necessary to use
 * the Addr_t ype within the end-user application as long as the type is not
 * used to interact with the kernel through system calls. Addr_t should be
 * declared as xAddr.
 *
 * @sa xAddr
 *
 ...
 * and is always represented in bytes. Size_t should be declared as xSize.
 *
 * @sa xSize
 *
 */
 ...
 * bound to any one unit of measure for time though most systems are configured
 * for millisecond resolution, milliseconds is not guaranteed and is dependent
 * on the system clock frequency and prescaler. Ticks_t should be declared as
 * xTicks.
 *
 * @sa xTicks
 *
 ...
 *
 * The Device_t data type is used as a device handle. The device handle is
 * created when xDeviceRegisterDevice() is called. For more information about
 * devices and device drivers, see xDeviceRegisterDevice() for more information.
 * Device_t should be declared as xDevice.
 *
 * @sa xDevice
 ...
 * when xTaskCreate() is called. For more information about tasks, see
 * xTaskCreate(). Task_t should be declared as xTask.
 *
 * @sa xTask
 * @sa xTaskCreate()
 * @attention The memory referenced by the task handle must be freed by calling
 * xTaskDelete().
 * @sa xTaskDelete()
 *
 */
 ...
 *
 * The StreamBuffer_t data type is used as a stream buffer handle. The stream
 * buffer handle is created when xStreamCreate() is called. For more information
 * about stream buffers, see xStreamCreate(). Stream_t should be declared as
 * xStream.
 *
 * @sa xStream
 * @sa xStreamCreate()
 * @attention The memory referenced by the stream buffer handle must be freed by
 * calling xStreamDelete().
 * @sa xStreamDelete()
 *
 */
 ...
 * when xQueueCreate() is called. For more information about queues, see
 * xQueueCreate(). Queue_t should be declared as xQueue.
 *
 * @sa xQueue
 * @sa xQueueCreate()
 * @attention The memory referenced by the queue handle must be freed by calling
 * xQueueDelete().
 * @sa xQueueDelete()
 *
 */
 ...
 *
 * The Timer_t data type is used as a timer handle. The timer handle is created
 * when xTimerCreate() is called. For more information about timers, see
 * xTimerCreate(). Timer_t should be declared as xTimer.
 *
 * @sa xTimer
 * @sa xTimerCreate()
 * @attention The memory referenced by the timer handle must be freed by calling
 * xTimerDelete().
 * @sa xTimerDelete()
 *
 */
 ...
 * to task notifcation may be received by event-driven and co-operative tasks
 ...
 * xTaskWait(). The TaskNotification_t type should be declared as
 * xTaskNotification.
 *
 * @sa xTaskNotification
 * @attention The memory allocated for the data struture must be freed by
 * calling xMemFree().
 * @sa xMemFree()
 * @sa xTaskNotifyGive()
 * @sa xTaskNotifyTake()
 * @sa xTaskWait()
 *
 * @attention The notification value is *NOT* null terminated and thus Standard
 * C Library string functions such as strcmp(), strcpy() and strlen(), which
 * expect a null terminated char array, must not be used to manipulate the
 * notification value.
 *
 */
  typedef struct TaskNotification_s {
    Base_t notificationBytes;                                /**< The length in
                                                              * bytes of the
                                                              * notification
                                                              * value which
                                                              * cannot exceed
 ...
 * and xTaskGetAllRuntimeStats() to obtain runtime statistics about a task. The
 * TaskRunTimeStats_t type should be declared as xTaskRunTimeStats.
 *
 ...
 */
  typedef struct TaskRunTimeStats_s {
    Base_t id;                                /**< The ID of the task referenced
                                               * by the task handle. */
 ...
 * xMemGetKernelStats() to obtain statistics about either memory region. The
 * MemoryRegionStats_t type should be declared as xMemoryRegionStats.
 *
 * @sa xMemoryRegionStats
 * @sa xMemGetHeapStats()
 * @sa xMemGetKernelStats()
 * @attention The memory allocated for the data structure must be freed by
 * calling xMemFree().
 * @sa xMemFree()
 *
 */
  typedef struct MemoryRegionStats_s {
    Word_t largestFreeEntryInBytes;                                 /**< The
                                                                     * largest
                                                                     * free
                                                                     * entry in
                                                                     * bytes. */
 ...
 * additional details about a task such as its ASCII name and state. The
 * TaskInfo_t structure is returned by xTaskGetTaskInfo() and
 * xTaskGetAllTaskInfo(). If only runtime statistics are needed, then
 * TaskRunTimeStats_t should be used because of its smaller memory footprint.
 * The TaskInfo_t should be declared as xTaskInfo
 *
 * @sa xTaskInfo
 * @sa xTaskGetTaskInfo()
 ...
 * calling xMemFree().
 *
 * @attention The task name is *NOT* null terminated and thus Standard C Library
 * string functions such as strcmp(), strcpy() and strlen(), which expect a null
 * terminated char array, must not be used to manipulate the task name.
 *
 */
 ...
 *
 * The QueueMessage_t stucture is used to store a queue message and is returned
 * by xQueueReceive() and xQueuePeek(). The QueueMessage_t stucture should be
 * declared as xQueueMessage.
 *
 * @sa xQueueMessage
 * @sa xQueueReceive()
 * @sa xQueuePeek()
 ...
 * calling xMemFree().
 *
 * @attention The message value is *NOT* null terminated and thus Standard C
 * Library string functions such as strcmp(), strcpy() and strlen(), which
 * expect a null terminated char array, must not be used to manipulate the
 * message value.
 *
 *
 */
  typedef struct QueueMessage_s {
    Base_t messageBytes;                            /**< The number of bytes
 ...
 *
 * The SystemInfo_t data structure is used to store information about the HeliOS
 * system and is returned by xSystemGetSystemInfo(). The SystemInfo_t structure
 * should be declared as xSystemInfo.
 *
 * @sa xSystemInfo
 ...
 * calling xMemFree().
 *
 * @attention The product name is *NOT* null terminated and thus Standard C
 * Library string functions such as strcmp(), strcpy() and strlen(), which
 * expect a null terminated char array, must not be used to manipulate the
 * product name.
 *
 ...
 *
 * The xDeviceRegisterDevice() system call, as part of the HeliOS device driver
 * model, registers a device driver with the HeliOS kernel. This system call
 * must be made before a device driver can be called by xDeviceRead(),
 * xDeviceWrite(), etc. If the device driver is successfully registered with the
 * kernel, xDeviceRegisterDevice() will return RETURN_SUCCESS. Once a device is
 * registered it cannot be un-registered - it can only be placed in a suspended
 * state which is done by calling xDeviceConfigDevice(). However, as with most
 * aspects of the device driver model in HeliOS, it is important to note that
 * the implementation of the device state and mode is up to the device driver
 * author. A quick word about device driver unique identifiers (uid). A device
 * driver uid *MUST* be a globally unique identifier. No two device drivers in
 * the same application can share the same uid. This is best achieved by
 * ensuring the device driver author selects a uid for his device driver that is
 * not in use by another device driver. A device driver template and
 * pre-packaged device drivers can be found in /drivers.
 *
 * @param  device_self_register_ A pointer to the self registration function for
 *                               the device driver.
 * @return                       xBase If the device driver is successfully
 *                               registered with the kernel,
 *                               xDeviceRegisterDevice() returns RETURN_SUCCESS.
 *                               Otherwise RETURN_FAILURE is returned..
 */
 ...
 *
 * The xDeviceIsAvailable() system call checks to see if a device driver is
 * "available", generally for a read, write or read/write operation. What
 * "available" means is up to the device driver author and may change based on
 * what mode the device driver is in. For example, if the device driver is in
 * read-only mode, then xDeviceIsAvailable() may return "true" if data is ready
 * to be read from the device. A device driver template and pre-packaged device
 * drivers can be found in /drivers.
 *
 * @param  uid_ The unique identifier of the device driver.
 * @return      xBase If the device driver is "available", then "true" is
 ...
 *
 * The xDeviceSimpleWrite() system call will write fixed length (one word) data
 * to a device driver. Whether the data is written is dependent on the device
 * driver mode, state and implementation of these features by the device driver
 * author. A device driver template and pre-packaged device drives can be found
 * in /drivers.
 *
 * @param  uid_  The unique identifier of the device driver
 * @param  data_ A pointer to the data to be written to the device - because
 *               xDeviceSimpleWrite() uses fixed length data, the pointer must
 *               reference a word of memory and the memory *MUST* be located in
 *               the heap memory region.
 * @return       xBase If the write operation was successful, RETURN_SUCCESS is
 *               returned. Otherwise RETURN_FAILURE is returned.
 */
 ...
 *
 * The xDeviceWrite() system call will write variable length data to a device
 * driver. Whether the data is written is dependent on the device driver mode,
 * state and implementation of these features by the device driver author. A
 * device driver template and pre-packaged drivers an be found in /drivers.
 *
 * @param  uid_  The unique identifier of the device driver.
 * @param  size_ The length (i.e., size) of the data to be written to the
 *               device.
 * @param  data_ A pointer to the data to be written to the device - because
 *               xDeviceWrite uses variable length data, the pointer must
 *               reference size_ bytes of memory and the memory *MUST* be
 *               located in the heap memory region.
 * @return       xBase If the write operation was successful, RETURN_SUCCESS is
 *               returned. Otherwise RETURN_FAILURE is returned.
 */
 ...
 *
 * The xDeviceSimpleRead() system call will read fixed length (one word) data
 * from a device driver. Whether the data is read is dependent on the device
 * driver mode, state and implementation of these features by the device driver
 * author. A device driver template and pre-packaged device drivers can be found
 * in /drivers.
 *
 * @param  uid_  The unique identifier of the device driver.
 * @param  data_ A pointer to the data buffer to read the data into from the
 *               device - because xDeviceSimpleRead() uses fixed length data,
 *               the pointer must reference a word of memory and the memory
 *               MUST* be located in the heap memory region.
 * @return       xBase If the read operation was successful, RETURN_SUCCESS is
 *               returned. Otherwise RETURN_FAILURE is returned.
 */
 ...
 *
 * The xDeviceRead() system call will read variable length data from a device
 * driver. Whether the data is read is dependent on the device driver mode,
 * state and implementation of these features by the device driver author. A
 ...
 * @param  data_ A pointer to the data buffer to read the data into from the
 *               device - because xDeviceRead() uses variable length data, the
 *               pointer must reference size_ bytes of memory and the memory
 *               MUST* be located in the heap memory region.
 * @return       xBase If the read operation was successful, RETURN_SUCCESS is
 *               returned. Otherwise RETURN_FAILURE is returned.
 */
 ...
 *
 * The xDeviceInitDevice() system call will initialize the device driver and its
 * device. Like most aspects of the HeliOS device driver model, the
 * implementation of this feature is dependent on the author of the device
 * driver. Some device drivers may not implement this feature of the device
 * driver model at all. A device driver template and pre-packaged drivers can be
 * found in /drivers.
 *
 * @param  uid_ The unique identifier of the device driver.
 * @return      xBase If the initialization of the device driver was successful,
 *              RETURN_SUCCESS is returned. Otherwise RETURN_FAILURE is
 *              returned.
 */
 ...
 *
 * The xDeviceConfigDevice() system call will configure the device driver and
 * its device. Like most aspects of the HeliOS device driver model, the
 * implementation of this feature is dependent on the author of the device
 * driver. Some device drivers may not implement this feature of the device
 * driver model at all. A device driver template and pre-packaged drivers can be
 * found in /drivers.
 *
 * @param  uid_    The unique identifier of the device driver.
 * @param  size_   The number of bytes (i.e., size) of the device configuration
 *                 data structure.
 * @param  config_ A pointer to the size_ bytes of memory occupied by the
 *                 configuration data structure - the memory *MUST* be located
 *                 in the heap memory region.
 * @return         xBase If configuration of the device driver was successful,
 *                 RETURN_SUCCESS is returned. Otherwise RETURN_FAILURE is
 *                 returned.
 */
 ...
 * settings. xMemAlloc() functions similarly to calloc() in that it clears the
 * memory it allocates.
 *
 ...
 * @sa xMemFree()
 *
 * @param  size_ The amount (size) of the memory to be allocated from the heap
 *               in bytes.
 * @return       xAddr If successful, xMemAlloc() returns the address of the
 *               newly allocated memory. If unsuccessful, the system call will
 *               return null.
 *
 * @note HeliOS technically does not allocate memory from what is traditionally
 * heap memory. HeliOS uses a private "heap" which is actually static memory
 * allocated at compile time. This is done to maintain MISRA C:2012 compliance
 * since standard library functions like malloc(), calloc() and free() are not
 * permitted.
 *
 */
 ...
 *
 * The xMemFree() system call will free heap memory allocated by xMemAlloc() and
 * other HeliOS system calls such as xSystemGetSystemInfo().
 *
 * @sa xMemAlloc()
 *
 * @param addr_ The address of the allocated heap memory to be freed.
 *
 * @warning xMemFree() cannot be used to free memory allocated for kernel
 * objects. Memory allocated by xTaskCreate(), xTimerCreate() or xQueueCreate()
 * must be freed by their respective delete system calls (e.g., xTaskDelete()).
 */
...
/**
 * @brief System call to return the amount of heap memory allcoated for a given
 * address.
 *
 * The xMemGetSize() system call returns the amount of heap memory in bytes that
 * is currently allocated to a specific address. If the address is null or
 * invalid, xMemGetSize() will return zero bytes.
 *
 * @param  addr_ The address of the allocated heap memory to obtain the size of
 *               the memory, in bytes, that is allocated.
 * @return       xSize The amount of memory currently allocated to the specific
 *               address in bytes. If the address is invalid or null,
 *               xMemGetSize() will return zero.
 *
 * @note If the address addr_ points to a structure that, for example, is 48
 * bytes in size base on sizeof(), xMemGetSize() will return the number of bytes
 * allocated by the block(s) that contain the structure. Assuming the default
 * block size of 32, a 48 byte structure would require TWO blocks so
 * xMemGetSize() would return 64 - not 48. xMemGetSize() also checks the health
 * of the heap and will return zero if it detects a consistency issue with the
 * heap. Thus, xMemGetSize() can be used to validate addresses before the
 * objects they reference are accessed.
 */
 ...
 *
 * The xMemGetHeapStats() system call will return statistics about the heap so
 * the end-user can better understand the state of the heap.
 *
 * @sa xMemoryRegionStats
 *
 ...
 *         if unsuccessful.
 *
 * @warning The memory allocated by xMemGetHeapStats() must be freed by
 * xMemFree().
 */
 ...
 *         if unsuccessful.
 *
 * @warning The memory allocated by xMemGetKernelStats() must be freed by
 * xMemFree().
 */
 ...
 *
 * @param  limit_ The message limit for the queue. When this number is reach,
 *                the queue is considered full and xQueueSend() will fail. The
 *                minimum limit for queues is dependent on the  setting
 *                CONFIG_QUEUE_MINIMUM_LIMIT.
 * @return        xQueue A queue is returned if successful, otherwise null is
 ...
 *
 * The xQueueDelete() system call will delete a message queue created by
 * xQueueCreate(). xQueueDelete() will delete a queue regardless of how many
 * messages the queue contains at the time xQueueDelete() is called. Any
 * messages the message queue contains will be deleted in the process of
 ...
 *
 * The xQueueGetLength() system call returns the length of the queue (the number
 * of messages the queue currently contains).
 *
 * @param  queue_ The queue to return the length of.
 * @return        xBase The number of messages in the queue. If unsuccessful or
 *                if the queue is empty, xQueueGetLength() returns zero.
 */
 ...
 *
 * The xQueueIsEmpty() system call will return a true or false dependent on
 * whether the queue is empty (message queue length is zero) or contains one or
 * more messages.
 *
 * @param  queue_ The queue to determine whether it is empty.
 * @return        xBase True if the queue is empty. False if the queue has one
 *                or more messages. xQueueIsQueueEmpty() will also return false
 *                if the queue parameter is invalid.
 */
 ...
 *
 * The xQueueIsFull() system call will return a true or false dependent on
 * whether the queue is full or contains zero messages. A queue is considered
 * full if the number of messages in the queue is equal to the queue's length
 * limit.
 *
 * @param  queue_ The queue to determine whether it is full.
 * @return        xBase True if the queue is full. False if the queue has zero.
 *                xQueueIsQueueFull() will also return false if the queue
 ...
 *
 * The xQueueMessageWaiting() system call returns true or false dependent on
 * whether there is at least one message waiting. The message queue does not
 * have to be full to return true.
 *
 * @param  queue_ The queue to determine whether one or more messages are
 *                waiting.
 * @return        xBase True if one or more messages are waiting. False if there
 *                are no messages waiting of the queue parameter is invalid.
 */
 ...
 *
 * The xQueueSend() system call will send a message using the specified message
 * queue. The size of the message value is passed in the message bytes
 * parameter. The maximum message value size in bytes is dependent on the
 * CONFIG_MESSAGE_VALUE_BYTES setting.
 *
 * @sa CONFIG_MESSAGE_VALUE_BYTES
 * @sa xQueuePeek()
 * @sa xQueueReceive()
 ...
 *
 * The xQueuePeek() system call will return the next message in the specified
 * message queue without dropping the message.
 *
 * @sa xQueueMessage
 * @sa xMemFree()
 *
 * @param  queue_ The queue to return the next message from.
 * @return        xQueueMessage The next message in the queue. If the queue is
 *                empty or the queue parameter is invalid, xQueuePeek() will
 *                return null.
 *
 ...
 *
 * The xQueueDropMessage() system call will drop the next message from the
 * message queue without returning the message.
 *
 ...
 *
 * The xQueueReceive() system call will return the next message in the message
 * queue and drop it from the message queue.
 *
 * @sa xQueueMessage
 * @sa xMemFree()
 *
 * @param  queue_ The queue to return the next message from.
 * @return        xQueueMessage The message returned from the queue. If the
 *                queue is empty of the queue parameter is invalid,
 *                xQueueReceive() will return null.
 *
 ...
 *
 * The xQueueLockQueue() system call will lock the message queue. Locking a
 * message queue will prevent xQueueSend() from sending messages to the queue.
 *
 ...
 *
 * The xQueueUnLockQueue() system call will unlock the message queue. Unlocking
 * a message queue will allow xQueueSend() to send messages to the queue.
 *
 ...
 *
 * The xStreamCreate() system call will create a new stream buffer. The memory
 * for a stream buffer is allocated from kernel memory and therefor cannot be
 * freed by calling xMemFree().
 *
 * @return xStreamBuffer The newly created stream buffer.
 *
 * @warning The stream buffer created by xStreamCreate() must be freed by
 * calling xStreamDelete().
 */
 ...
 *
 * The xStreamDelete() system call will delete a stream buffer and free its
 * memory. Once a stream buffer is deleted, it can not be written to or read
 * from.
 *
 ...
 *
 * The xStreamSend() system call will write one byte to the stream buffer. If
 * the stream buffer's length is equal to CONFIG_STREAM_BUFFER_BYTES (i.e.,
 * full) then the byte will not be written to the stream buffer and
 * xStreamSend() will return RETURN_FAILURE.
 *
 * @param  stream_ The stream buffer to operate on.
 * @param  byte_   The byte to be sent to the stream buffer.
 * @return         xBase Returns RETURN_SUCCESS if the byte was successfully
 *                 written to the stream buffer. Otherwise, returns
 *                 RETURN_FAILURE.
 */
...
/**
 * @brief The xStreamReceive() system call will return the contents of the
 * stream buffer.
 *
 * The xStreamReceive() system call will return the contents of the stream
 * buffer. The contents are returned as a byte array whose length is known by
 * the bytes_ paramater. Because the byte array is stored in the heap, it must
 * be freed by calling xMemFree().
 *
 * @param  stream_ The stream to operate on.
 * @param  bytes_  The number of bytes returned (i.e., length of the byte array)
 *                 by xStreamReceive().
 * @return         xByte* The byte array containing the contents of the stream
 *                 buffer.
 *
 * @warning The byte array returned by xStreamReceive() must be freed by calling
 * xMemFree().
 */
...
/**
 * @brief The xStreamBytesAvailable() system call returns the length of the
 * stream buffer.
 *
 * The xStreamBytesAvailable() system call will return the length of the stream
 * buffer in bytes (i.e., bytes available to be received by xStreamReceive()).
 *
 * @param  stream_ The stream to operate on.
...
/**
 * @brief The xStreamIsEmpty() system call returns true if the stream buffer is
 * empty.
 *
 * The xStreamIsEmpty() system call is used to determine if the stream buffer is
 * empty. A stream buffer is considered empty when it's length is equal to zero.
 * If the buffer is greater than zero in length, xStreamIsEmpty() will return
 * false.
 *
 * @param  stream_ The stream buffer to operate on.
 * @return         xBase Returns true if the stream buffer length is equal to
 *                 zero in length, otherwise xStreamIsEmpty() will return false.
 */
  xBase xStreamIsEmpty(const xStreamBuffer stream_);


/**
 * @brief The xStreamIsFull() system call returns true if the stream buffer is
 * full
 *
 * The xStreamIsFull() system call is used to determine if the stream buffer is
 * full. A stream buffer is considered full when it's length is equal to
 * CONFIG_STREAM_BUFFER_BYTES. If the buffer is less than
 * CONFIG_STREAM_BUFFER_BYTES in length, xStreamIsFull() will return false.
 *
 * @param  stream_ The stream buffer to operate on.
 * @return         xBase Returns true if the stream buffer is equal to
 *                 CONFIG_STREAM_BUFFER_BYTES in length, otherwise
 *                 xStreamIsFull() will return false.
 */
...
/**
 * @brief The xSystemGetSystemInfo() system call will return information about
 * the running system.
 *
 ...
 * states.
 *
 * @return xSystemInfo The system info is returned if successful, otherwise null
 *         is returned if unsuccessful.
 *
 * @sa xSystemInfo
 * @sa xMemFree()
 *
 * @warning The memory allocated by the xSystemGetSystemInfo() must be freed
 * with xMemFree().
 */
 ...
 *
 * @param  name_          The ASCII name of the task which can be used by
 *                        xTaskGetHandleByName() to obtain the task handle. The
 *                        length of the name is depended on the
 *                        CONFIG_TASK_NAME_BYTES. The task name is NOT a null
 *                        terminated char string.
 * @param  callback_      The address of the task main function. This is the
 *                        function that will be invoked by the scheduler when a
 *                        task is scheduled for execution.
 * @param  taskParameter_ A pointer to any type or structure that the end-user
 *                        wants to pass into the task as a parameter. The task
 *                        parameter is not required and may simply be set to
 *                        null.
 ...
 * @sa CONFIG_TASK_NAME_BYTES
 *
 * @warning xTaskCreate() MUST be called outside the scope of the HeliOS
 * scheduler (i.e., not from a task's main). The task memory can only be freed
 * by xTaskDelete().
 */
 ...
 *
 * @warning xTaskDelete() MUST be called outside the scope of the HeliOS
 * scheduler (i.e., not from a task's main).
 */
 ...
 * task specified by its ASCII name. The length of the task name is dependent on
 * the CONFIG_TASK_NAME_BYTES setting. The name is compared byte-for-byte so the
 * name is case sensitive.
 *
 * @sa CONFIG_TASK_NAME_BYTES
 *
 * @param  name_ The ASCII name of the task to return the handle of. The task
 *               name is NOT a null terminated string.
 * @return       xTask The task handle. xTaskGetHandleByName() returns null if
 ...
 *
 * The xTaskGetHandleById() system call will return the task handle of the task
 * specified by identifier identifier.
 *
 * @sa xBase
 *
 * @param  id_ The identifier of the task to return the handle of.
 * @return     xTask The task handle. xTaskGetHandleById() returns null if the
 ...
 *
 * The xTaskGetAllRunTimeStats() system call will return the runtime statistics
 * for all of the tasks regardless of their state. The xTaskGetAllRunTimeStats()
 * system call returns the xTaskRunTimeStats type. An xBase variable must be
 * passed by reference to xTaskGetAllRunTimeStats() which will be updated by
 * xTaskGetAllRunTimeStats() to contain the number of tasks so the end-user can
 * iterate through the tasks. The xTaskRunTimeStats memory must be freed by
 * xMemFree() after it is no longer needed.
 *
 * @sa xTaskRunTimeStats
 * @sa xMemFree()
 *
 * @param  tasks_ A variable of type xBase passed by reference which will
 *                contain the number of tasks upon return. If no tasks currently
 *                exist, this variable will not be modified.
 * @return        xTaskRunTimeStats The runtime stats returned by
 *                xTaskGetAllRunTimeStats(). If there are currently no tasks
 *                then this will be null. This memory must be freed by
 *                xMemFree().
 *
 * @warning The memory allocated by xTaskGetAllRunTimeStats() must be freed by
 * xMemFree().
 */
 ...
 *
 * The xTaskGetTaskRunTimeStats() system call returns the task runtime
 * statistics for one task. The xTaskGetTaskRunTimeStats() system call returns
 ...
 *
 * @warning The memory allocated by xTaskGetTaskRunTimeStats() must be freed by
 * xMemFree().
 */
 ...
 * @return       xTaskInfo The xTaskInfo structure containing the task details.
 *               xTaskGetTaskInfo() returns null if the task cannot be found.
 *
 * @warning The memory allocated by xTaskGetTaskInfo() must be freed by
 * xMemFree().
 */
 ...
 * containing the details of ALL tasks including their identifier, name, state
 ...
 *                contain the number of tasks upon return. If no tasks currently
 *                exist, this variable will not be modified.
 * @return        xTaskInfo The xTaskInfo structure containing the tasks
 *                details. xTaskGetAllTaskInfo() returns null if there no tasks
 *                or if a consistency issue is detected.
 *
 * @warning The memory allocated by xTaskGetAllTaskInfo() must be freed by
 * xMemFree().
 */
 ...
 * @return       xTaskState The xTaskState of the task. If the task cannot be
 *               found, xTaskGetTaskState() will return null.
 */
 ...
 * is NOT a null terminated char string. The memory allocated for the char array
 * must be freed by xMemFree() when no longer needed.
 *
 * @sa CONFIG_TASK_NAME_BYTES
 * @sa xMemFree()
 *
 * @param  task_ The task to return the name of.
 * @return       xChar* A pointer to the char array containing the ASCII name of
 *               the task. The task name is NOT a null terminated char string.
 *               xTaskGetName() will return null if the task cannot be found.
 *
 ...
 * @return       xBase The identifier of the task. If the task cannot be found,
 *               xTaskGetId() returns zero (all tasks identifiers are 1 or
 *               greater).
 */
 ...
 *
 * The xTaskNotifyStateClear() system call will clear a waiting direct to task
 ...
 * depending on whether there is a direct to task notification waiting for the
 * task.
 *
 * @param  task_ The task to check for a waiting task notification.
 * @return       xBase Returns true if there is a task notification. False if
 *               there is no notification or if the task could not be found.
 */
 ...
 *
 * The xTaskNotifyGive() system call will give a direct to task notification to
 * the specified task. The task notification bytes is the number of bytes
 * contained in the notification value. The number of notification bytes must be
 * between one and the CONFIG_NOTIFICATION_VALUE_BYTES setting. The notification
 * value must contain a pointer to a char array containing the notification
 * value. If the task already has a waiting task notification, xTaskNotifyGive()
 * will NOT overwrite the waiting task notification.
 * xTaskNotifyGive() will return true if the direct to task notification was
 * successfully given.
 *
 ...
 *
 * The xTaskNotifyTake() system call will return the waiting direct to task
 * notification if there is one. The xTaskNotifyTake() system call will return
 * an xTaskNotification structure containing the notification bytes and its
 * value. The memory allocated by xTaskNotifyTake() must be freed by xMemFree().
 *
 * @sa xTaskNotification
 * @sa xTaskNotifyGive()
 * @sa xMemFree()
 ...
 * @return       xTaskNotification The xTaskNotification structure containing
 *               the notification bytes and value. xTaskNotifyTake() will return
 *               null if no waiting task notification exists or if the task
 *               cannot be found.
 *
 * @warning The memory allocated by xTaskNotifyTake() must be freed by
 * xMemFree().
 */
 ...
 * in the waiting state, the task will be executed every period_ ticks. Changing
 * the period to zero will prevent the task from being executed even if it is in
 * the waiting state unless it were to receive a direct to task notification.
 *
 * @sa xTaskWait()
 * @sa xTaskGetPeriod()
 * @sa xTaskResetTimer()
 ...
 * @return       xTicks The timer period in ticks. xTaskGetPeriod() will return
 *               zero if the timer period is zero or if the task could not be
 *               found.
 */
 ...
 * This system call will not return until xTaskSuspendAll() is called. If
 * xTaskSuspendAll() is called, xTaskResumeAll() must be called before
 * xTaskStartScheduler() can be called again to continue executing tasks.
 *
 */
 ...
 *
 * The xTaskResumeAll() system call will set the scheduler state to running so
 * the next call to xTaskStartScheduler() will resume execute of all tasks. The
 * state of each task is not altered by xTaskSuspendAll() or xTaskResumeAll().
 *
 * @sa xTaskSuspendAll()
 *
 */
 ...
 * so the scheduler will stop and return. The state of each task is not altered
 * by xTaskSuspendAll() or xTaskResumeAll().
 *
 * @sa xTaskResumeAll()
 *
 */
 ...
 * scheduler. The state of the scheduler can only be changed using
 * xTaskSuspendAll() and xTaskResumeAll().
 *
 * @sa xSchedulerState
 * @sa xTaskSuspendAll()
 * @sa xTaskResumeAll()
...
/**
 * @brief The xTaskChangeWDPeriod() will change the period on the task watchdog
 * timer.
 *
 ...
 * @param period_ The task watchdog timer period which is measured in ticks. If
 *                zero, the task watchdog timer will not have any effect.
 */
 ...
 *
 * The xTaskGetWDPeriod() will return the current task watchdog timer for the
 * task.
 *
 * @sa xTaskChangeWDPeriod()
 * @param  task_ The task to get the task watchdog timer period for.
 * @return       xTicks  The task watchdog timer period which is measured in
 *               ticks.
 */
 ...
 * @return         xTimer The newly created timer. If the timer period parameter
 *                 is less than zero or xTimerCreate() was unable to allocate
 *                 the required memory, xTimerCreate() will return null.
 *
 ...
 * @param period_ The timer period in is ticks. Timer period must be zero or
 *                greater.
 */
 ...
 * @return        xTicks The timer period. If the timer cannot be found,
 *                xTimerGetPeriod() will return zero.
 */
 ...
 * @return        xBase True if active, false if not active or if the timer
 *                could not be found.
 */
 ...
 * xTimerHasTimerExpired() will NOT reset the timer. Timers will not
 * automatically reset. Timers MUST be reset with xTimerReset().
 *
 * @sa xTimerReset()
 *
 * @param  timer_ The timer to determine if the period has expired.
 * @return        xBase True if the timer has expired, false if the timer has
 *                not expired or could not be found.
 */
 ...
 *
 * The xTimerStart() system call will place the timer in the running (active)
 * state. Neither xTimerStart() nor xTimerStop() will reset the timer. Timers
 * can only be reset with xTimerReset().
 *
 * @sa xTimerStop()
 * @sa xTimerReset()
 *
 ...
 * Neither xTimerStart() nor xTimerStop() will reset the timer. Timers can only
 * be reset with xTimerReset().
 *
 * @sa xTimerStart()
 * @sa xTimerReset()
 *
