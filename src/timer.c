...
Base_t xTimerHasTimerExpired(Timer_t *timer_) {
  ...
  if (RETURN_SUCCESS == _TimerListFindTimer_(timer_)) {
    ...
    if ((TimerStateRunning == timer_->state) && (zero < timer_->timerPeriod) && ((SYSTICK() - timer_->timerStartTime) > timer_->timerPeriod)) {
