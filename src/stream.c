...
/* The xStreamDelete() system call will delete a stream. Streams are fast for
 * sending streams of bytes between tasks. */
void xStreamDelete(const StreamBuffer_t *stream_) {
  /* Assert if the heap fails its health check or if the stream pointer the
   * end-user passed is invalid. */
  SYSASSERT(ISSUCCESSFUL(__MemoryRegionCheckKernel__(stream_, MEMORY_REGION_CHECK_OPTION_W_ADDR)));


  /* Check if the heap is health and the stream pointer the end-user passed is
...
/* The xStreamSend() system call will send one byte to the designated stream.
 * Sending to a stream must be done one byte at a time. However, receiving from
 * a stream will return all available bytes in a stream. */
Base_t xStreamSend(StreamBuffer_t *stream_, const Byte_t byte_) {
  ...
  if(ISSUCCESSFUL(__MemoryRegionCheckKernel__(stream_, MEMORY_REGION_CHECK_OPTION_W_ADDR))) {
     ...
     * CONFIG_STREAM_BUFFER_BYTES in length before we attempt to write a byte to
     * it. */
    if(CONFIG_STREAM_BUFFER_BYTES > stream_->length) {
      stream_->length++;


      /* Offset the stream buffer length by -1 because array is base zero. */
...
Byte_t *xStreamReceive(const StreamBuffer_t *stream_, HalfWord_t *bytes_) {
  ...
  /* Check if the bytes parameter is null. It can't be null because we need to
   * return the number of bytes received. */
...
/* The xStreamBytesAvailable() system call will return the number of waiting
 * bytes in the stream. */
...
/* The xStreamReset() system call will reset the stream and clear its buffer of
 * all waiting bytes. */
...
/* The xStreamIsEmpty() system call will return true if the stream buffer is
 * empty (i.e., zero in size). */
...
/* The xStreamIsFull() system call will return true if the stream buffer is full
 * (i.e., size is equal to CONFIG_STREAM_BUFFER_BYTES) */
