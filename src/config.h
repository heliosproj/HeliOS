 ...
 * __ArduinoAssert__(). The arduino.cpp file can be found in the /extras
 * directory. It must be copied into the /src directory to be built.
 *
 *
 * @note On some MCU's like the 8-bit AVRs, it is necessary to undefine the
 * DISABLE_INTERRUPTS() macro because interrupts must be enabled to write to the
 * serial bus.
 *
 */
/*
 ...
 * bytes. The literal must be appended with "u" to maintain MISRA C:2012
 * compliance.
 *
 * @sa xQueueMessage
 *
 ...
 * the ASCII task name. The larger the size of the task name, the greater impact
 * there will be on system performance. The default size is 8 bytes. The literal
 * must be appended with "u"
 ...
 * By default the CONFIG_MEMORY_REGION_SIZE_IN_BLOCKS is defined on a per
 * platform and/or tool-chain basis therefor it is not defined here by default.
 * The literal must be appended with a "u" to maintain MISRA C:2012 compliance.
 *
 */
 ...
 * memory regions. The default value is 32 bytes. The literal must be appended
 ...
 * also not accept messages from xQueueSend(). The default value is 5. The
 ...
 * xStreamSend(). The default value is 32. The literal must be appended with "u"
 * to maintain MISRA C:2012 compliance.
 *
 */
