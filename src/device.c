...
#include "config.h"
      ...
      if(__PointerIsNotNull__(dlist)) {
        ...
        if(!OK(__DeviceListFind__(uid_, &device))) {
          ...
          if(__PointerIsNull__(device)) {
            /* Allocate kernel memory for the device structure; then, if all
             * goes well, populate the structure with all of the device details.
             */
            if(OK(__KernelAllocateMemory__((volatile Addr_t **) &device, sizeof(Device_t)))) {
              if(__PointerIsNotNull__(device)) {
                if(OK(__memcpy__(device->name, name_, CONFIG_DEVICE_NAME_BYTES))) {
                  ...
                  /* If this is the first device added to the device list, then
                   * go ahead and set the device list head to the device.
                   * Otherwise we need to traverse the list until we reach the
                   * end. */
  ...
  Return_t xDeviceIsAvailable(const HalfWord_t uid_, Base_t *res_) {
    ...
    if(__DeviceUidNonZero__() && __PointerIsNotNull__(res_) && __PointerIsNotNull__(dlist)) {
      ...
      if(OK(__DeviceListFind__(uid_, &device))) {
        if(__PointerIsNotNull__(device)) {
          /* Set the result paramater of xDeviceIsAvailable() to the value of
           * the device structure available member.
           *
      ...
      if(OK(__DeviceListFind__(uid_, &device))) {
        if(__PointerIsNotNull__(device)) {
          ...
          if(((DeviceModeReadWrite == device->mode) || (DeviceModeReadOnly == device->mode)) && (DeviceStateRunning == device->state)) {
             ...
             * check that the data returned by the device driver is waiting for
             * us in kernel memory. */
    ...
    if(__DeviceUidNonZero__() && __PointerIsNotNull__(size_) && __PointerIsNotNull__(data_) && __PointerIsNotNull__(dlist)) {
      ...
      if(OK(__DeviceListFind__(uid_, &device))) {
        if(__PointerIsNotNull__(device)) {
           ...
           * readable. */
          if(((DeviceModeReadWrite == device->mode) || (DeviceModeReadOnly == device->mode)) && (DeviceStateRunning == device->state)) {
            /* Call the device driver's DEVICENAME_read() function and check
             * that the data returned by the device driver is waiting for us in
             * kernel memory. */
 ...
 */
        ...
        if(__ObjectIsValid__(device)) {
          ...
          if(((DeviceModeReadWrite == device->mode) || (DeviceModeReadOnly == device->mode)) && (DeviceStateRunning == device->state)) {
            ...
            if(OK((*device->read)(device, size_, data_))) {
              ...
              /* No assertion - device read errors are normal operational
               * failures
               */
  ...
  Return_t xDeviceInitDevice(const HalfWord_t uid_) {
    ...
    if(__DeviceUidNonZero__() && __PointerIsNotNull__(dlist)) {
      ...
      if(OK(__DeviceListFind__(uid_, &device))) {
        if(__PointerIsNotNull__(device)) {
          /* Call the device drivers DEVICENAME_init() function to initialize
           * the device.
           *
