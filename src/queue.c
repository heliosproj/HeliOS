...
void xQueueDelete(Queue_t *queue_) {
  ...
  passed is invalid. */
  SYSASSERT(RETURN_SUCCESS == MemoryRegionCheckHeap(queue_, MEMORY_CHECK_REGION_OPTION_W_ADDR));
  ...
  if (RETURN_SUCCESS == MemoryRegionCheckHeap(queue_, MEMORY_CHECK_REGION_OPTION_W_ADDR)) {
