...
void xQueueDelete(Queue_t *queue_) {
  ...
  passed is invalid. */
  SYSASSERT(RETURN_SUCCESS == MemoryRegionCheckHeap(queue_, MEMORY_REGION_CHECK_OPTION_W_ADDR));
  ...
  if (RETURN_SUCCESS == MemoryRegionCheckHeap(queue_, MEMORY_REGION_CHECK_OPTION_W_ADDR)) {
