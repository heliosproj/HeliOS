...
/*UNCRUSTIFY-ON*/
   ...
   * @details Configuration structure for initializing an STM32 USART peripheral with hardware instance and communication parameters.
   */
  typedef struct USARTSTMInitConfig_s {

  #if !defined(POSIX_ARCH_OTHER)

      USART_TypeDef *usartInstance;  /**< Pointer to STM32 USART hardware instance (e.g., USART1, USART2) */
    ...
    Word_t baudRate;                 /**< Baud rate for serial communication (e.g., 9600, 115200) */
   ...
   * @details Initializes internal buffers, configures USART hardware, and enables interrupts.
   *
   ...
   * @details Configures USART parameters such as baud rate, data bits, parity, and stop bits.
   *
   * @param[in,out] device_ Pointer to device structure
   * @param[in,out] size_ Pointer to configuration data size
   * @param[in] config_ Pointer to configuration data (USARTSTMInitConfig_t or CharIOConfig_t)
   *
